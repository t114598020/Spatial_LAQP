...
We reproduced LAQP and compared it with SAQP (sampling-based AQP) and AQP++ (pre-aggregate AQP with range similar) on POWER and Uber Pickups NYC dataset, experiments show that LAQP has better performance than SAQP and AQP++, but will cost more time because it used error prediction model.

The images below are the ARE (Average Relative Error) results evaluated on POWER and Uber Pickups NYC dataset.
